...
try {
    ...
    Set-ItemProperty -Path $regPath -Name "MMTaskbarMode" -Value 2 -Type DWord -Force
    
    # MMTaskbarGlomLevel: 0 = Always combine, 1 = Combine when full, 2 = Never combine
    Set-ItemProperty -Path $regPath -Name "MMTaskbarGlomLevel" -Value 1 -Type DWord -Force
